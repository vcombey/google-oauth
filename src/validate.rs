...
pub fn validate_id_token_info<S: AsRef<str>>(
    client_id: S,
    parser: &JwtParser<GooglePayload>,
) -> anyhow::Result<()> {
...
pub fn do_validate(cert: &Cert, parser: &JwtParser<GooglePayload>) -> anyhow::Result<()> {
    match parser.header.alg.as_str() {
        "RS256" => validate_rs256(&cert, parser.msg().as_str(), parser.sig.as_slice())?,
        "ES256" => bail!("id_token: unimplemented alg: ES256"),
        a => bail!(
            "id_token: expected JWT signed with RS256 or ES256, but found {}",
            a
        ),
    };
...
pub fn validate_rs256(cert: &Cert, msg: &str, sig: &[u8]) -> anyhow::Result<()> {
    ...
    verifying_key.verify(msg.as_bytes(), &Signature::try_from(sig)?)?;
